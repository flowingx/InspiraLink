...
本项目实现一个“睡眠呼吸监测 + 低风险辅助呼吸演示”的半实物系统。系统通过 BMP280/AHT20 采集采样管内的微小压差，用 MAX30102 展示血氧和心率趋势，用舵机挤压简易球囊完成台架上的辅助泵气动作，并通过 Flask + ECharts 实时展示数据、报警和控制状态。
...
| 呼吸检测 | BMP280 + AHT20 | I2C, 气压/温湿度 | 3.5-5 CNY | 捕捉采样管内吸气负压和呼气正压 |
...
BMP280 不直接暴露在鼻孔或气流口下，而是放入密封小盒作为采样腔。盒体开两个孔：一个引出 I2C 线并用热熔胶密封，另一个连接输液软管。软管末端用于台架模拟呼吸输入，可用手捏软管、注射器或小气囊模拟吸气负压。

为降低高频气流扰动，可在软管中加入极少量棉花或海绵作为物理滤波。测试时先采集静息数据，确认自然漂移和噪声范围，再确定阈值。
...
1. 传感采集层：20Hz 读取 BMP280/AHT20，5-10Hz 读取 MAX30102。
...
默认启用模拟硬件模式，便于在电脑上演示前端和算法状态。上树莓派后可将 `simulate_hardware` 改为 `False`，并补齐 MAX30102 适配器。
