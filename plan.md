...
- BMP280/AHT20 位于浅采样腔或小盒子内部，可加薄纱布或透气膜降低冷凝水影响；不建议裸露放置传感器，因为环境气流会造成持续负压或慢漂移。
- 不堵死呼气孔，避免影响面罩排气。
- 采样腔需要有小泄放孔或透气膜，让湿度在无呼吸时逐步回落，但不能直接暴露在外部风中。
- 杜邦线改为焊接小转接板或带锁扣线，线沿面罩边缘和绑带固定，减小拉扯。
- 算法同时使用压差、湿度变化和温度变化判断呼吸活动，但湿度/温度只认上升沿，压力只认短脉冲，持续负压或环境漂移不算有效呼吸。
...
- MAX30102 到货前不参与控制，也不显示假数据。
- MAX30102 到货后可作为“泵气有效性评估”信号：若呼吸暂停期间已经完成多次辅助泵气，但 SpO2 仍低或继续下降，应升级报警并触发轻度唤醒提示，提醒用户恢复自主呼吸；若 SpO2 逐步恢复到安全范围，则说明台架辅助动作在趋势上有效，应暂停继续泵气并回到观察状态。
- SpO2 不应作为单点瞬时触发源，需要结合连续趋势、读数质量、近期泵气次数和呼吸活动状态判断，避免 MAX30102 低可靠读数导致误动作。
- 手动泵气测试有冷却时间和次数限制。
