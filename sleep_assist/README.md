...
默认 `assist_mode` 为 `apnea_only`：正常呼吸只记录并清除呼吸暂停报警，不泵气；超过 `apnea_detect_seconds` 没有检测到有效呼吸时进入报警，并按 `assist_interval_seconds` 周期泵气。每次泵气后会在 `pump_artifact_ignore_seconds` 内忽略压差触发，降低球囊动作对 BMP280 的干扰。
