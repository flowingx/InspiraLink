...
- 传感器不应裸露放在环境气流中。实测发现裸露放置会导致持续负压、慢漂移和湿度下降被误判；放进带小泄放孔/透气膜的盒式采样腔后更接近真实呼吸信号。
...
- 呼吸活动不只看单个负压尖峰。湿度/温度只认上升沿，下降沿不算呼吸；压力必须形成短时脉冲并回落，持续负压或环境漂移会被标记为 `pressure_drift_ignored`。
- 若 AHT20 可用，湿度上升是优先证据；若只有压力信号，必须满足短脉冲条件。
...
- 不要把湿度下降、温度下降、持续负压或单调漂移当成呼吸。当前设计只认湿度/温度上升沿和压力短脉冲。
